...
from lean.components.util.json_modules_handler import (
    build_and_configure_modules,
    config_build_for_name,
    interactive_config_build,
    non_interactive_config_build_for_name,
)
from lean.components.util.live_utils import (
    _configure_initial_cash_interactively,
    _configure_initial_holdings_interactively,
    configure_initial_cash_balance,
    configure_initial_holdings,
    get_last_portfolio_cash_holdings,
)
from lean.components.util.name_rename import (
    rename_internal_config_to_user_friendly_format,
)
from lean.constants import DEFAULT_ENGINE_IMAGE
from lean.container import container
from lean.models.cli import (
    cli_addon_modules,
    cli_brokerages,
    cli_data_downloaders,
    cli_data_queue_handlers,
    cli_history_provider,
)
...
@option(
    ...
    type=Choice(
        [
            dp.get_name()
            for dp in cli_data_downloaders
            if dp.get_id() != "TerminalLinkBrokerage"
        ],
...
@option(
    ...
    show_default=True,
    default=False,
    help="Show secrets as they are input",
)
@option("--addon-module", type=str, multiple=True, hidden=True)
@option("--extra-config", type=(str, str), multiple=True, hidden=True)
@option(
    "--extra-docker-config",
    type=str,
    default="{}",
...
def deploy(
    ...
    **kwargs,
    ...
    if environment is not None and (
        brokerage is not None or len(data_provider_live) > 0
    ):
        raise RuntimeError(
            "--environment and --brokerage + --data-provider-live are mutually exclusive"
        )
    ...
    if environment is not None:
        ...
        lean_config = lean_config_manager.get_complete_lean_config(
            environment_name, algorithm_file, None
        )
        ...
        if environment_name in lean_config["environments"]:
            ...
            for key in [
                ...
                if key not in lean_environment:
                    raise MoreInfoError(
                        f"The '{environment_name}' environment does not specify a {rename_internal_config_to_user_friendly_format(key)}",
            ...
            logger.debug(
                f"Deploy(): loading env '{environment_name}'. Brokerage: '{brokerage}'. IDQHs: "
                f"{data_provider_live}. HistoryProviders: {history_providers}"
            )
        else:
            logger.info(
                f"Environment '{environment_name}' not found, creating from scratch"
            )
    ...
    if brokerage:
        ...
        brokerage_instance = non_interactive_config_build_for_name(
            lean_config, brokerage, cli_brokerages, kwargs, logger, environment_name
        )
    else:
        # let the user choose the brokerage
        brokerage_instance = interactive_config_build(
            lean_config,
            cli_brokerages,
            logger,
            kwargs,
    ...
    if data_provider_live and len(data_provider_live) > 0:
        for data_feed_name in data_provider_live:
            data_feed = non_interactive_config_build_for_name(
                lean_config,
                data_feed_name,
                cli_data_queue_handlers,
                kwargs,
        ...
        data_provider_live_instances = interactive_config_build(
            lean_config,
            cli_data_queue_handlers,
            logger,
            kwargs,
    ...
    data_provider_live = [
        provider.get_name() for provider in data_provider_live_instances
    ]
    if data_provider_historical is None:
        data_provider_historical = "Local"
    data_downloader_instances = non_interactive_config_build_for_name(
        lean_config,
    ...
    engine_image, container_module_version, project_config = (
        container.manage_docker_image(image, update, no_update, algorithm_file.parent)
    )

    organization_id = container.organization_manager.try_get_working_organization_id()
    paths_to_mount = {}
    for module in (
        data_provider_live_instances
        + [data_downloader_instances, brokerage_instance]
    ...
    if not lean_config["environments"][environment_name]["live-mode"]:
        raise MoreInfoError(
            f"The '{environment_name}' is not a live trading environment (live-mode is set to false)",
    ...
    if python_venv is not None and python_venv != "":
        lean_config["python-venv"] = (
            f"{'/' if python_venv[0] != '/' else ''}{python_venv}"
        )
    ...
    cash_balance_option, holdings_option, last_cash, last_holdings = (
        get_last_portfolio_cash_holdings(
            ...
            brokerage_instance,
            project_config.get("cloud-id", None),
            project,
        )
    )
    ...
    if (
        environment is None and brokerage is None
    ):  # condition for using interactive panel
        if cash_balance_option != LiveInitialStateInput.NotSupported:
            live_cash_balance = _configure_initial_cash_interactively(
                logger, cash_balance_option, last_cash
            )

        if holdings_option != LiveInitialStateInput.NotSupported:
            live_holdings = _configure_initial_holdings_interactively(
                logger, holdings_option, last_holdings
            )
    else:
        if cash_balance_option != LiveInitialStateInput.NotSupported:
            live_cash_balance = configure_initial_cash_balance(
                logger, cash_balance_option, live_cash_balance, last_cash
            )
        elif live_cash_balance is not None and live_cash_balance != "":
            raise RuntimeError(
                f"Custom cash balance setting is not available for {brokerage_instance}"
            )

        if holdings_option != LiveInitialStateInput.NotSupported:
            live_holdings = configure_initial_holdings(
                logger, holdings_option, live_holdings, last_holdings
            )
        elif live_holdings is not None and live_holdings != "":
            raise RuntimeError(
                f"Custom portfolio holdings setting is not available for {brokerage_instance}"
            )
    ...
    if live_holdings:
        lean_config["live-holdings"] = [
            {
                "Symbol": {"Value": holding["symbol"], "ID": holding["symbolId"]},
    ...
    lean_config["algorithm-id"] = (
        f"L-{output_config_manager.get_live_deployment_id(output, given_algorithm_id)}"
    )
    ...
    build_and_configure_modules(
        ...
        logger,
        environment_name,
        container_module_version,
    )

    if container.platform_manager.is_host_arm():
        if "InteractiveBrokersBrokerage" in lean_config["environments"][
            environment_name
        ]["live-mode-brokerage"] or any(
            "InteractiveBrokersBrokerage" in dataQueue
            for dataQueue in lean_config["environments"][environment_name][
                "data-queue-handler"
            ]
        ):
            raise RuntimeError(
                f"InteractiveBrokers is currently not supported for ARM hosts"
            )

    # --- IBKR: allow "connect to existing TWS/IBGW" mode when credentials are empty ---
    # Default/original behavior stays unchanged when both username and password are provided.
    ib_user = str(lean_config.get("ib-user-name", "") or "").strip()
    ib_pwd = str(lean_config.get("ib-password", "") or "").strip()

    has_user = bool(ib_user)
    has_pwd = bool(ib_pwd)

    # If only one is set, fail fast (prevents confusing partial configs)
    if has_user ^ has_pwd:
        raise RuntimeError(
            "Interactive Brokers configuration error: provide BOTH --ib-user-name and --ib-password, "
            "or leave BOTH empty to connect to an existing TWS/IBGateway instance."
        )

    # If both empty -> connect-only mode (no automated login)
    if not has_user and not has_pwd:
        # Ensure empty values are explicitly present
        lean_config["ib-user-name"] = ""
        lean_config["ib-password"] = ""

        # If user kept default 127.0.0.1, it points to the container itself when using bridge networks.
        # Use host.docker.internal to reach the Docker host (works on Linux because we already add extra_hosts).
        env_cfg = lean_config.get("environments", {}).get(environment_name, {})
        ib_host = env_cfg.get("ib-host") or lean_config.get("ib-host") or "127.0.0.1"

        if ib_host in ("127.0.0.1", "localhost", ""):
            # Prefer host.docker.internal for bridge/custom networks (like "lean_cli")
            env_cfg["ib-host"] = "host.docker.internal"
            lean_config["environments"][environment_name] = env_cfg

        # Optional: log what we’re doing (safe, no secrets)
        env_live = lean_config.get("environments", {}).get(environment_name, {})
        logger.debug(
            f"IBKR connect-only mode: using ib-host={env_live.get('ib-host')} ib-port={env_live.get('ib-port')} (credentials empty)"
        )
    # --- end IBKR connect-only mode ---
