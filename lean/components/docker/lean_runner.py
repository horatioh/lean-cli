...
class LeanRunner:
    ...
    def run_lean(
        ...
        run_options = self.get_basic_docker_config(
            lean_config,
            algorithm_file,
            output_dir,
            debugging_method,
        ...
        # --- PATCH START: map HostConfig.NetworkMode -> docker-py network_mode ---
        # If the user passed a low-level HostConfig (e.g. via --extra-docker-config),
        # map HostConfig.NetworkMode to the high-level docker SDK key 'network_mode'.
        # Also ensure we do not keep an incompatible 'network' when using host networking.
        try:
            host_cfg = run_options.get("HostConfig")
            if isinstance(host_cfg, dict):
                nm = host_cfg.get("NetworkMode")
                if nm:
                    # put the docker-py expected key
                    run_options["network_mode"] = nm

                    # when explicit host/network_mode is provided, prefer it over any previously set 'network'
                    if "network" in run_options:
                        run_options.pop("network", None)

            # Remove HostConfig so we don't forward low-level HostConfig to docker-py run (unsupported)
            if "HostConfig" in run_options:
                run_options.pop("HostConfig", None)
        except Exception:
            # Defensive: don't break the deployment flow if extra_docker_config was unexpected
            self._logger.debug(
                "Failed to map HostConfig.NetworkMode to network_mode (non-fatal)."
            )
        # --- PATCH END ---
        self._logger.debug(
            f"DEBUG_LEAN adjusted docker run options: network={run_options.get('network')} "
            f"network_mode={run_options.get('network_mode')} restart_policy={run_options.get('restart_policy')}"
        )

        # Set up PTVSD or DebugPy debugging
        if (
            debugging_method == DebuggingMethod.PTVSD
            or debugging_method == DebuggingMethod.DebugPy
        ):
        ...
        if debugging_method == DebuggingMethod.LocalPlatform:
            run_options["ports"]["5678"] = (
                "0"  # Using port 0 will assign a random port every time
            )
        ...
        if debugging_method == DebuggingMethod.LocalPlatform:
            ...
            port = self._docker_manager.get_container_port(
                run_options["name"], "5678/tcp"
            )
            output_config = self._output_config_manager.get_output_config(output_dir)
    ...
    def get_basic_docker_config(
        ...
        run_options = self._initialize_run_options(
            detach, docker_project_config, debugging_method
        )

        self._mount_common_directories(
            run_options,
            paths_to_mount,
            lean_config,
            data_dir,
        ...
        # Set up modules
        set_up_common_csharp_options_called = self._setup_installed_packages(
            run_options, image, lean_config
        )

        # Set up language-specific run options
        self.setup_language_specific_run_options(
            run_options,
            project_dir,
            algorithm_file,
            set_up_common_csharp_options_called,
            release,
            image,
        )
    ...
    def get_basic_docker_config_without_algo(
        self,
        lean_config: Dict[str, Any],
        debugging_method: Optional[DebuggingMethod],
        detach: bool,
        ...
        self._mount_common_directories(
            run_options, paths_to_mount, lean_config, data_dir, storage_dir, None, None
        )
        ...
        self._mount_lean_config_and_finalize(
            run_options, lean_config, None, config_local_path
        )
    ...
    def _mount_lean_config_and_finalize(
        ...
        run_options["mounts"].append(
            Mount(
                target=f"{LEAN_ROOT_PATH}/config.json",
        ...
        run_options["name"] = lean_config.get(
            "container-name", f"lean_cli_{str(uuid4()).replace('-', '')}"
        )
        ...
        if output_dir:
            ...
            if "environment" in lean_config and "environments" in lean_config:
                ...
                if "live-mode-brokerage" in environment:
                    output_config.set(
                        "brokerage", environment["live-mode-brokerage"].split(".")[-1]
                    )

    def _setup_installed_packages(
        self,
        run_options: Dict[str, Any],
        image: DockerImage,
        lean_config: Dict[str, Any],
        target_path: str = "/Lean/Launcher/bin/Debug",
    ):
        """Sets up installed packages."""
        installed_packages = self._module_manager.get_installed_packages()
        if installed_packages:
            self._logger.debug(
                f"LeanRunner._setup_installed_packages(): installed packages {len(installed_packages)}"
            )
            ...
            framework_ver = self._docker_manager.get_image_label(
                image, "target_framework", DEFAULT_LEAN_DOTNET_FRAMEWORK
            )
            run_options["commands"].append(
                f"dotnet new classlib -o /ModulesProject -f {framework_ver} --no-restore"
            )
            ...
            for package in installed_packages:
                ...
                run_options["commands"].append(
                    f"rm -rf /root/.nuget/packages/{package.name.lower()}"
                )
    ...
    def _mount_common_directories(
        ...
        files_to_mount = [
            ...
            (
                "terminal-link-symbol-map-file",
                cli_root_dir / DEFAULT_DATA_DIRECTORY_NAME / "symbol-properties",
            ),
        ]
        for key, base_path in files_to_mount:
            ...
            local_path = (
                lean_config_entry
                if lean_config_entry.is_absolute()
                else base_path / lean_config_entry
            )
    ...
    def _initialize_run_options(
        self,
        detach: bool,
        docker_project_config: Dict[str, Any],
        debugging_method: Optional[DebuggingMethod],
    ):
        """
    ...
    def _handle_data_providers(self, lean_config: Dict[str, Any], data_dir: Path):
        ...
        if (
            ...
            self._force_disk_provider_if_necessary(
                lean_config,
                "map-file-provider",
    ...
    def set_up_python_options(
        self, project_dir: Path, run_options: Dict[str, Any], image: DockerImage
    ) -> None:
        ...
        source_files = [
            file.relative_to(project_dir).as_posix() for file in source_files
        ]
        ...
        requirements_files = list(library_dir.rglob("requirements.txt")) + [
        ...
        requirements_txt = (
            self._temp_manager.create_temporary_directory() / "requirements.txt"
        )
        ...
        python_version = self._docker_manager.get_image_label(
            image, "python_version", DEFAULT_LEAN_PYTHON_VERSION
        )
        site_packages_path = DOCKER_PYTHON_SITE_PACKAGES_PATH.replace(
            "{LEAN_PYTHON_VERSION}", python_version
        )

        # Mount a volume to the user packages directory so we don't install packages every time
        site_packages_volume = self._docker_manager.create_site_packages_volume(
            requirements_txt
        )
    ...
    def set_up_csharp_options(
        ...
        for path in compile_root.rglob("*.csproj"):
            self._ensure_csproj_is_valid(
                compile_root, path, csproj_temp_dir, run_options, framework_ver
            )
        ...
        if existing_no_warn is not None:
            codes = [
                c for c in split(r"[^a-zA-Z0-9]", existing_no_warn.text) if c != ""
            ]
        ...
        msbuild_properties = ";".join(
            f"{key}={value}" for key, value in msbuild_properties.items()
        )
        run_options["commands"].append(
            f'dotnet build "/LeanCLI/{relative_project_file}" "-p:{msbuild_properties}"'
        )
    ...
    def set_up_common_csharp_options(
        self, run_options: Dict[str, Any], target_path: str = "/Lean/Launcher/bin/Debug"
    ) -> None:
        """
...
for library_id, library_data in project_assets["targets"][project_target].items():
    ...
    for key, value in library_data.get("contentFiles", {}).items():
        ...
        run_options["mounts"].extend(
            [
                ...
                Mount(
                    target="/copy_csharp_dependencies.py",
    ...
    def _ensure_csproj_is_valid(
        ...
        if net_framework:
            ...
            if target_framework_iter:
                target_frameworks = [
                    framework.text for framework in target_framework_iter
                ]
                if target_frameworks:
                    if net_framework not in target_frameworks:
                        raise RuntimeError(
                            f"This project is targeting {target_frameworks[0].replace('net', 'Net ')}"
        ...
        for package_reference in csproj.iter("PackageReference"):
            if (
                not package_reference.get("Include", "")
                .lower()
            ...
            package_reference.append(
                self._xml_manager.parse("<Private>False</Private>")
            )
        ...
        run_options["mounts"].append(
            Mount(
                target=f"/LeanCLI/{csproj_path.relative_to(compile_root).as_posix()}",
    ...
    def _force_disk_provider_if_necessary(
        self,
        lean_config: Dict[str, Any],
        config_key: str,
        zip_provider: str,
        ...
        zip_names = sorted(
            [f.name for f in zip_dir.iterdir() if f.name.endswith(".zip")], reverse=True
        )
        zip_names = [sub(r"[^\d]", "", name) for name in zip_names]

        if (
            len(zip_names) == 0
            or (datetime.now() - datetime.strptime(zip_names[0], "%Y%m%d")).days > 7
        ):
            lean_config[config_key] = disk_provider

    def setup_language_specific_run_options(
        self,
        run_options,
        project_dir,
        algorithm_file,
    ...
    def mount_project_and_library_directories(
        self, project_dir: Path, run_options: Dict[str, Any]
    ) -> None:
        # Mount the project directory
        run_options["volumes"][str(project_dir)] = {"bind": "/LeanCLI", "mode": "rw"}
    ...
    def mount_paths(self, paths_to_mount, lean_config, run_options):
        ...
        for key, pathStr in paths_to_mount.items():
            ...
            mounts.append(
                Mount(target=target, source=str(path), type="bind", read_only=True)
            )
            environment[key] = target

    @staticmethod
    def parse_extra_docker_config(
        run_options: Dict[str, Any], extra_docker_config: Optional[Dict[str, Any]]
    ) -> None:
        # Add known additional run options from the extra docker config.
        # For now, only device_requests is supported
        if extra_docker_config is not None:
            # --- PATCH START: support HostConfig.NetworkMode + RestartPolicy (docker-py style) ---
            # docker SDK containers.run expects 'network_mode' and 'restart_policy'
            # but users may pass low-level Docker JSON under 'HostConfig'.
            host_cfg = extra_docker_config.get("HostConfig")
            if isinstance(host_cfg, dict):
                network_mode = host_cfg.get("NetworkMode")
                if network_mode:
                    run_options["network_mode"] = network_mode
                    # Avoid conflicting settings: if network_mode is set, don't also set 'network'
                    run_options.pop("network", None)

                restart_policy = host_cfg.get("RestartPolicy")
                if isinstance(restart_policy, dict) and restart_policy:
                    # docker-py expects e.g. {"Name": "no"} or {"Name": "unless-stopped"} etc.
                    run_options["restart_policy"] = restart_policy

            # Also accept top-level network_mode / restart_policy (more docker-py native)
            if (
                "network_mode" in extra_docker_config
                and extra_docker_config["network_mode"]
            ):
                run_options["network_mode"] = extra_docker_config["network_mode"]
                run_options.pop("network", None)

            if (
                "restart_policy" in extra_docker_config
                and extra_docker_config["restart_policy"]
            ):
                run_options["restart_policy"] = extra_docker_config["restart_policy"]
            # --- PATCH END ---
            ...
            if "environment" in extra_docker_config:
                ...
                if isinstance(extra_docker_config["environment"], list):
                    target.update(
                        {
                            ...
                            for item in [
                                ...
                                for item in extra_docker_config["environment"]
                            ]
                        }
                    )
                    ...
                    raise ValueError(
                        "Additional environment variables can be passed to the container in a dictionary, list of '{key}={value}' strings, or list of '(key, value)' tuples."
                    )
    ...
    def _ensure_iqconnect_running(
        self, lean_config: Dict[str, Any], data_provider_package_name: str
    ) -> None:
        """
        ...
        args = [
            ...
            "-product",
            lean_config["iqfeed-productName"],
            "-version",
